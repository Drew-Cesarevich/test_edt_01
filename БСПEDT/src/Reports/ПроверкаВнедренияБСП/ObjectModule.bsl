	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБТС, "2.0.10.1") >= 0 Тогда
		...
		Для каждого ОбъектМетаданных Из РезультатПроверки.НеразделенныхДанныхНетВКонтролирующейПодписке Цикл
			ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Контроль неразделенных данных'"), 
				РезультатПроверки.ТекстИсключенияНеразделенныхДанных);
		КонецЦикла;
		Для каждого ОбъектМетаданных Из РезультатПроверки.ОбъектыСНесколькимиРазделителями Цикл
			ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Контроль неразделенных данных'"), 
				РезультатПроверки.ТекстИсключенияСНесколькимиРазделителями);
		КонецЦикла;
...
Функция ТаблицаДопустимыхВызововБСПВОбработчикахСобытийКорневыхМодулейКонфигурации()
	...
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "СтандартныеПодсистемыКлиент.ПередНачаломРаботыСистемы";
	
	НоваяСтрокаВызова = ТаблицаДопустимыхВызововБСП.Добавить();
	НоваяСтрокаВызова.ИмяКорневогоМодуля = "МодульУправляемогоПриложения";
	НоваяСтрокаВызова.ИмяОбработчикаСобытия = "ПриНачалеРаботыСистемы";
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "СтандартныеПодсистемыКлиент.ПриНачалеРаботыСистемы";
	
	НоваяСтрокаВызова = ТаблицаДопустимыхВызововБСП.Добавить();
	НоваяСтрокаВызова.ИмяКорневогоМодуля = "МодульУправляемогоПриложения";
	НоваяСтрокаВызова.ИмяОбработчикаСобытия = "ПередЗавершениемРаботыСистемы";
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "СтандартныеПодсистемыКлиент.ПередЗавершениемРаботыСистемы";
...
Процедура ПроверитьОбластьПеременныхМодуляПриложения(ТекстМодуля, ИмяМодуля)
	...
	ТекстОбластьОписаниеПеременных = ВРег("#ОбластьОписаниеПеременных");
	...
	Для Каждого СтрокаПроверяемогоОбработчика Из ТаблицаПроверяемыхОбработчиков Цикл
		...
		Для НомерСтроки = НомерСтрокиНачалаОбработчика + 1 По НомерСтрокиКонцаОбработчика - 1 Цикл
			...
			Если ПустаяСтрока(СтрокаОбработчика)
				ИЛИ СтрНачинаетсяС(СтрокаОбработчика, "/")
		...
		Для Каждого СтрокаДопустимогоВызова Из ТаблицаДопустимыхВызововПоОбработчику Цикл
			...
			ДопустимыйВызовПроцедурыБСП = ДопустимыйВызовПроцедурыБСП + "(";
		...
		ТекстОбработчика = СокрЛП(ТекстОбработчика);
		ПерваяСтрокаНедопустимогоВызова = СтрПолучитьСтроку(ТекстОбработчика, 1);
		ПерваяСтрокаНедопустимогоВызова = СокрЛП(ПерваяСтрокаНедопустимогоВызова);
		
		НомерСтрокиНедопустимогоВызова = НомерСтрокиНачалаОбработчика;
		
		ПозицияНедопустимогоВызова = СтрНайти(ТекстМодуля, ПерваяСтрокаНедопустимогоВызова);
