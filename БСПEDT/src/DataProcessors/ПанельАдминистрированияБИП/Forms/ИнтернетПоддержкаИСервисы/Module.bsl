...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияПодсистемБИП.ПриСозданииФормыИнтернетПоддержкаИСервисы(ЭтотОбъект);
	
	НастройкиПрограммыБИППереопределяемый.ПриСозданииФормыИнтернетПоддержкаИСервисы(ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИнтернетПоддержкаПодключена" Тогда
		...
		Если ВведенныеДанныеАутентификации <> Неопределено Тогда
			...
			ОтобразитьСостояниеПодключенияИПП(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДлительностьОперацииПлатежнойСистемыПриИзменении(Элемент)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ИнтеграцияСПлатежнымиСистемами") Тогда
		МодульИнтеграцияСПлатежнымиСистемамиВызовСервера = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"ИнтеграцияСПлатежнымиСистемамиВызовСервера");
		МодульИнтеграцияСПлатежнымиСистемамиВызовСервера.УстановитьДлительностьОперации(
...
Процедура ВариантОбновленияРегламентированныхОтчетовПриИзменении(Элемент)
	...
	Если ВариантОбновленияРегламентированныхОтчетов = 1
		...
		Если Не ИнтернетПоддержкаПользователейКлиент.ДоступноПодключениеИнтернетПоддержки() Тогда
			ПоказатьПредупреждение(,
				НСтр("ru = 'Для автоматического обновления регламентированных отчетов необходимо
					|подключить Интернет-поддержку пользователей.'"));
			Возврат;
		КонецЕсли;
...
&НаКлиенте
Процедура ПроверкаКонтрагентовПроверитьДоступКВебСервису(Команда)
...
&НаКлиенте
Процедура НастройкаИнтеграцииСПлатежнымиСистемами(Команда)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ИнтеграцияСПлатежнымиСистемами") Тогда
		МодульИнтеграцияСПлатежнымиСистемамиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"ИнтеграцияСПлатежнымиСистемамиКлиент");
		МодульИнтеграцияСПлатежнымиСистемамиКлиент.НастройкиИнтеграции(ЭтотОбъект);
	КонецЕсли;
...
Процедура ОткрытьШаблоныСообщенийСБП(Команда)
	...
	Если (ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ИнтеграцияСПлатежнымиСистемами")
		...
		МодульИнтеграцияСПлатежнымиСистемамиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"ИнтеграцияСПлатежнымиСистемамиКлиент");
		МодульИнтеграцияСПлатежнымиСистемамиКлиент.ОткрытьШаблоныСообщенийСБП(УникальныйИдентификатор);
...
&НаСервере
Процедура ПриИзмененииКонстантыБСПНаСервере(ИмяКонстанты)
...
Процедура ОтобразитьСостояниеПодключенияИПП(Форма)
	...
	Иначе
		...
		Если ИнтернетПоддержкаПользователейКлиентСервер.ЛогинСодержитНеДопустимыеСимволы(Логин) <> "" Тогда
...
Процедура ОбновитьСостояниеИнтернетПоддержки()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		Элементы.ГруппаНовости.Видимость = ЭтоАдминистраторСистемы
			И (Не РазделениеВключено
			Или Не ДоступноИспользованиеРазделенныхДанных);
		Если Элементы.ГруппаНовости.Видимость Тогда
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ИнтеграцияСПлатежнымиСистемами") Тогда
		...
		МодульИнтеграцияСПлатежнымиСистемамиСлужебный = ОбщегоНазначения.ОбщийМодуль(
			"ИнтеграцияСПлатежнымиСистемамиСлужебный");
		ПараметрыПодсистемыИнтеграцияСПлатежнымиСистемами =
			МодульИнтеграцияСПлатежнымиСистемамиСлужебный.ПараметрыОкруженияФормыИнтернетПоддержкаИСервисы();
		Если ПараметрыПодсистемыИнтеграцияСПлатежнымиСистемами.НастройкаИнтеграцияДоступна Тогда
			ДлительностьОперацииПлатежнойСистемы =
				ПараметрыПодсистемыИнтеграцияСПлатежнымиСистемами.ДлительностьОперацииПлатежнойСистемы;
		Иначе
			Элементы.ГруппаИнтеграцияСПлатежнымиСистемами.Видимость = Ложь;
		КонецЕсли;
...
Процедура ОбновитьСостояниеПодсистемыОнлайнОплаты()
	...
	Если Не ПараметрыПодсистемы.НастройкаИнтеграцииДоступна
			И Не ПараметрыПодсистемы.ЕстьПравоПросмотраСтатусовОнлайнОплат Тогда
		Элементы.ГруппаОнлайнОплаты.Видимость = Ложь;
		Возврат;
	ИначеЕсли Не ПараметрыПодсистемы.НастройкаИнтеграцииДоступна Тогда
