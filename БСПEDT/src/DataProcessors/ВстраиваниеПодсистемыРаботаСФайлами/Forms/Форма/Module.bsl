...
Процедура ВыгрузитьКонфигурациюВXML(РабочийКаталог, Пользователь, Пароль)
	...
	Если КаталогКопии1CD <> Неопределено Тогда
		...
		Исключение
			// АПК:280 Если сейчас каталог еще занят другим процессом, то он будет удален позднее автоматически.
		КонецПопытки;
...
Функция ТекстОбласти(ТекстМодуля, ИмяОбласти)
	...
	Если НачалоОбласти > 0 Тогда
		...
		ТекстОбласти = Лев(ТекстОбласти, СтрДлина(ТекстОбласти) - СтрДлина(ТекстПоиска) 
			+ СтрДлина("#КонецОбласти") - 3);
...
Функция ТекстМетода(ТекстОбластиПоиска, Заголовок)
	...
	Если НачалоМетода > 0 Тогда
		...
		ТекстМетода        = Лев(ТекстМетода, ОкончаниеПроцедуры + ?(СтрНачинаетсяС(ИмяМетодаНРег, "процедура"), 
			СтрДлина("КонецПроцедуры") -1, СтрДлина("КонецФункции") - 1));
...
Функция ДобавитьМетодВОбласть(ТекстОбласти, ИмяОбласти, Заголовок, Параметры, Директива, ВНачало)
	...
	Если ВНачало Тогда
		...
		НачалоОбласти  = Лев(ТекстОбласти, СтрДлина(ИмяОбласти) + СтрДлина("#Область") + 1); // плюс пробел 
	...
	Иначе
		...
		ОкончаниеОбласти = Прав(ТекстОбласти, СтрДлина("#КонецОбласти"));
		ТекстДляЗамены   = ТекстДобавления + Символы.ПС + Символы.ПС + ОкончаниеОбласти;
		ТекстОбласти     = Лев(ТекстОбласти, СтрДлина(ТекстОбласти) - СтрДлина("#КонецОбласти")) + ТекстДляЗамены;
...
Функция ДополнитьМетод(ТекстМетода, ТекстВставки)
	...
	ОкончаниеМетода = Прав(ТекстМетода, ?(СтрЗаканчиваетсяНа(НРег(ТекстМетода), НРег("КонецПроцедуры")), 
		СтрДлина("КонецПроцедуры"), СтрДлина("КонецФункции")));
