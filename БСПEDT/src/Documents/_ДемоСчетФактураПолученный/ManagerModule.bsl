	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Исключение
				...
				ОтменитьТранзакцию();
				ПодробнаяИнформацияПоПроблемам.Вставить(ДокументСсылка, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Результат = Запрос.Выполнить().Выгрузить(); // @skip-check query-in-loop - порционное проведение документов.
