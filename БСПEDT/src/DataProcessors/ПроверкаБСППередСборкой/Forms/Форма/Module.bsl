		...
		Если Проверка.ПроводитьПроверку Тогда
			Попытка
				...
				Для Каждого ПромежуточнаяПроверка Из МассивПроверок Цикл
					...
					ПрименитьИсключенияПроверки(СтруктураОшибок);
	...
	Для Каждого ТипМетаданныхВСписке Из СписокТиповМетаданных Цикл
		...
		Для Каждого ОбъектМетаданныхЗаданногоТипа Из Метаданные[ТипМетаданных] Цикл
			Если ТипМетаданных = "ПараметрыСеанса"
				...
				Или СтрНачинаетсяС(ОбъектМетаданныхЗаданногоТипа.Имя, "КлючиДоступаКРегистру_Демо")
				Или СтрНачинаетсяС(ОбъектМетаданныхЗаданногоТипа.Имя, "Удалить_Демо") Тогда
...
Функция Подключаемый_ПроверитьКомментарииДляБлоковКодаПодсистем()
	...
	МассивПодсистем = СписокИспользуемыхПодсистем(Ложь);
	МассивМодулей = НайтиФайлы(КаталогВыгрузкиМодулей, "*.bsl", Истина);	
	ТекстФайла = Новый ТекстовыйДокумент;
...
Функция Подключаемый_НайтиНерекомендуемыеНастройкиРолей()
	...
	Если ЗначениеЗаполнено(Результат.НекорректнаяНастройкаПрав) Тогда
		Пояснение = НСтр("ru = '1.1. См. пункт 1 стандарта ""Проверка прав доступа"" https://its.1c.eu/db/v8std#content:737:hdoc
...
Функция ЭтоПоставляемыйОбъект(ОбъектМетаданных, ТипМетаданных)
	...
	Иначе
		ИсключениеПроверки = СтрНачинаетсяС(ОбъектМетаданных.Имя, "_Демо");
	КонецЕсли;
...
Функция ПлатформеннаяПроверкаКонфигурации()
	...
	Исключение
		...
		СписокОшибочныхОбъектов.Добавить(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), "Обработка.ПроверкаБСППередСборкой");
		Возврат СписокОшибочныхОбъектов;
	КонецПопытки;
...
&НаСервере
Процедура ПрименитьИсключенияПроверки(СтруктураОшибок)
	
	Если ТипЗнч(СтруктураОшибок) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(СтруктураОшибок.ОшибочныеДанные) <> Тип("СписокЗначений") Тогда
		Возврат;
	КонецЕсли;
	
	НовыеОшибочныеДанные = Новый СписокЗначений;
	Для Каждого Элемент Из СтруктураОшибок.ОшибочныеДанные Цикл
		Если ИсключаемыеОбъекты.НайтиПоЗначению(Элемент.Представление) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		НовыеОшибочныеДанные.Добавить(Элемент.Значение, Элемент.Представление);
		
	КонецЦикла;
	
	СтруктураОшибок.ОшибочныеДанные = НовыеОшибочныеДанные;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьИсключенияПроверки()
	
	МакетИсключений = РеквизитФормыВЗначение("Объект").ПолучитьМакет("ИсключаемыеИзПроверкиПодсистемы");
	Подсистемы = Новый Массив;
	Для НомерСтроки = 1 По МакетИсключений.КоличествоСтрок() Цикл
		Строка = МакетИсключений.ПолучитьСтроку(НомерСтроки);
		Подсистема = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Строка);
		Если Подсистема = Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	ЗаполнитьИсключаемыеОбъекты(Подсистемы);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьИсключаемыеОбъекты(Подсистемы)
	
	Если ИсключаемыеОбъекты = Неопределено Тогда
		ИсключаемыеОбъекты = Новый СписокЗначений;
	КонецЕсли;
	
	Для Каждого Подсистема Из Подсистемы Цикл
		Для Каждого ОбъектПодсистемы Из Подсистема.Состав Цикл
			ИсключаемыеОбъекты.Добавить(ОбъектПодсистемы.ПолноеИмя());
		КонецЦикла;
		
		ЗаполнитьИсключаемыеОбъекты(Подсистема.Подсистемы);
	КонецЦикла;
