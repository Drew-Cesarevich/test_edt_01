...
Процедура ЗаполнитьСоответствиеОбъектовПодсистемамПоСписку(Соответствие, КоллекцияПодсистем = Неопределено)
	...
	Для Каждого ЭлементПодсистема Из КоллекцияПодсистем Цикл
		...
		ЭлементПодсистема = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ЭлементПодсистема);
...
Функция СтрокаСсылкаНаСтандарты()
	...
	Иначе
		Возврат "<a href=""http://its.1c.eu/db/v8std"">""Системе стандартов и методик разработки конфигураций для платформы 1С:Предприятие 8""</a>"; //@Non-NLS
	КонецЕсли;
