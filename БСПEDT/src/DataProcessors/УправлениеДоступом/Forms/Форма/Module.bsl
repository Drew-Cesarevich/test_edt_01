...
#Если Не ВебКлиент Тогда
	...
	Если ИсточникСравнения = "Файл" Тогда
		// АПК:441-выкл Данные требуются после вызова для анализа.
...
Процедура ЗаполнитьЗаголовкиИПодсказки()
	...
	Элементы.Пояснение3.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.Пояснение3.Заголовок,
		"ПриЗаполненииПоставляемыхПрофилейГруппДоступа",
		"УправлениеДоступомПереопределяемый");
	
	Элементы.Пояснение4.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.Пояснение4.Заголовок,
		"ПриЗаполненииВидовОграниченийПравОбъектовМетаданных",
		"УправлениеДоступомПереопределяемый");
	
	Элементы.СформироватьРазличияОписанийВидовДоступаПоПравам.РасширеннаяПодсказка.Заголовок
		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сформировать описание различий между новым описанием и
	...
	Элементы.СформироватьОписаниеВидовДоступаПоПравам.РасширеннаяПодсказка.Заголовок
		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сформировать полное описание для вставки в процедуру %1 общего модуля %2.'"),
			"ПриЗаполненииВидовОграниченийПравОбъектовМетаданных",
			"УправлениеДоступомПереопределяемый");
...
Функция СвойстваСписка(Список, ДополнительныеПараметры = Неопределено)
	...
	Исключение
		...
		ОшибкаОписанияОграничения = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		Результат = Неопределено;
	КонецПопытки;
	
	Если Результат <> Неопределено
	   И ЗначениеЗаполнено(Результат.ОшибкаОписанияОграничения) Тогда
		
		ОшибкаОписанияОграничения = Результат.ОшибкаОписанияОграничения;
		Результат = Неопределено;
	КонецЕсли;
...
Функция ОграниченияДоступаИзТабличногоДокумента(ОшибкиВВыгрузке)
	...
	Для НомерСтроки = 2 По ВсеОграниченияДоступа.ВысотаТаблицы Цикл
		...
		МетаданныеТаблицы = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Свойства.Таблица);
...
Процедура ВыгрузитьТекстыОграниченийИзФайлаКонфигурации()
	...
	ЗапуститьПредприятие(ПараметрыЗапускаКлиента);
...
#Иначе
	...
	КодВозврата = 0;
	Попытка
		// АПК:534-выкл Строка запуска безопасна.
		ЗапуститьПриложение(СтрокаЗапуска,, Истина, КодВозврата);
		// АПК:534-вкл
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось запустить:
			|%1
			|по причине: 
			|%2'"), 
			СтрокаЗапуска, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
