...
Функция ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, ТДРезультатаЗапроса, ПараметрыВыводаЗапроса, ОтчетПоВыполнениюЗапроса, МеткаЗапроса) Экспорт
	...
	Если ПараметрыВыводаЗапроса.ВыводитьВременныеТаблицы Тогда
...
Функция ПолучитьИмяВременнойТаблицы(ТекстЗапроса, Буфер, Позиция)
	...
	Для Индекс = Позиция По ДлинаТекста Цикл
	...
	Для ИндексВременнойТаблицы = Индекс По ДлинаТекста Цикл
	...
	Возврат НСтр("ru = 'Временная таблица'") + ": " + ИмяТаблицы;
...
Функция ВывестиРезультатЗапросов(ТДРезультатаЗапроса, МассивДанныхПоЗапросу, ПараметрыЗапроса, ПараметрыВыводаЗапроса, ОтчетПоВыполнениюЗапроса)
	...
	Поместить        = "ПОМЕСТИТЬ"; // @Query-part
	...
	Для Индекс = 0 По КоличествоТекстовЗапросов - 1 Цикл
		...
		Если ПозицияПоместить <> 0 И Не ПараметрыВыводаЗапроса.ВыводитьВременныеТаблицы Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ПозицияПоместить <> 0 Тогда
			...
			ИмяЗапроса = ПолучитьИмяВременнойТаблицы(ТекстЗапросаМассива, БуферПоместить, ПозицияПослеПоместить);
...
Процедура ЗаписатьКонфигурациюВФайл(СтруктураФайлаКонфигурации, ПараметрыФайлаКонфигурации, РезультатВключения, Записывать)
	...
	Если Записывать Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ru = 'Не удалось записать конфигурационный файл в каталоге %1
				|по причине:
				|%2
				|
				|Проверьте права доступа.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", ПараметрыФайлаКонфигурации.КаталогКонфигурацииПриложения);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2", ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			РезультатВключения.Вставить("Результат", Ложь);
			РезультатВключения.Вставить("Причина", ТекстСообщения);
		КонецПопытки;
