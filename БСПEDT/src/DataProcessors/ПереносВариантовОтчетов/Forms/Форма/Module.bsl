...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Хранилище <> Неопределено И Хранилище.Имя = "ХранилищеВариантовОтчетов" Тогда
		...
		Элементы.ЗагрузитьВариантыОтчетов.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
...
Функция СохранитьВариантыОтчетовНаСервере()
	...
	Исключение
		ПредставлениеОшибки = НСтр("ru = 'Не удалось сохранить варианты отчетов по причине:
			|%1'");
		ЗаписатьОшибкуПоВарианту(Неопределено, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеОшибки, 
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеОшибки, 
...
Функция ЗагрузитьВариантыОтчетовНаСервере()
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ПредставлениеОшибки = НСтр("ru = 'Не удалось загрузить варианты отчетов по причине:
			|%1'");
		ЗаписатьОшибкуПоВарианту(Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеОшибки, 
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеОшибки, 
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке())));
		Результат = НСтр("ru = 'Не удалось загрузить настройки вариантов отчетов.
...
Функция НачатьКонвертациюВариантовОтчетов()
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ПредставлениеОшибки = НСтр("ru = 'Не удалось получить варианты отчетов из стандартного хранилища по причине:'")
				+ Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписатьОшибкуПоВарианту(Неопределено, ПредставлениеОшибки);
		КонецПопытки;
		...
		ОтчетМетаданные = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ВыборкаХранилища.КлючОбъекта);
...
&НаСервере
Процедура ЗаписатьОшибкуПоВарианту(Вариант, Сообщение, Реквизит1 = Неопределено, Реквизит2 = Неопределено, Реквизит3 = Неопределено)
	Уровень = УровеньЖурналаРегистрации.Ошибка;
	ЗаписатьЖурнал(Уровень, Вариант, Сообщение, Реквизит1, Реквизит2, Реквизит3);
КонецПроцедуры

&НаСервере
Процедура ЗаписатьЖурнал(Уровень, Ссылка, Текст, Параметр1 = Неопределено, Параметр2 = Неопределено, Параметр3 = Неопределено)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Перенос вариантов отчетов'", ОбщегоНазначения.КодОсновногоЯзыка()),
		Уровень, ЭтотОбъект().Метаданные(), Ссылка, Текст);
КонецПроцедуры
