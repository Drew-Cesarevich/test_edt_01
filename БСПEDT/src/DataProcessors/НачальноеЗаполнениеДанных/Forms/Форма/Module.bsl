	...
	Для каждого СтрокаТаблицы Из ТаблицаДанныхОбласти Цикл
		...
		ИначеЕсли ЭтоОписаниеТипов (ТипРеквизитаСтруктуры) Тогда
			...
			Для каждого ТипСтрокой Из МассивСтрок Цикл
				...
				МетаданныеПоТипу = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ТипСтрокой);
...
Процедура ОбъектМетаданныхПриИзмененииНаСервере()
	...
	МетаданныеОбъекта = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Объект.ОбъектМетаданных);
		...
		Для Каждого СтрокаТаблицы Из ТаблицаИсточник Цикл
			...
			Если Не СтрокаТаблицы.Предопределенный Тогда
				Если СтрНайти(СтрокаТаблицы.Ссылка.Метаданные().ВводПоСтроке, "Наименование") > 0
					И ЭтоУстаревшийЭлемент(СтрокаТаблицы.Наименование) Тогда
			...
			Если ЭтоУстаревшийЭлемент(СтрокаТаблицы.ИмяПредопределенныхДанных) Тогда
				УдаляемыеСтроки.Добавить(СтрокаТаблицы);
			КонецЕсли;
	...
	Для каждого СтрокаДанных Из ИсточникДанных Цикл
		...
		Для каждого Реквизит Из РеквизитыИнициализации Цикл
			...
			Иначе//Если ЭтоСсылочныйРеквизит Тогда
				...
				Если Метаданные.ПланыОбмена.Найти(МетаданныеРеквизита.Имя) <> Неопределено Тогда
				...
				ИначеЕсли Метаданные.Справочники.Найти(МетаданныеРеквизита.Имя) <> Неопределено
					Или Метаданные.ПланыВидовХарактеристик.Найти(МетаданныеРеквизита.Имя) <> Неопределено
					Или Метаданные.ПланыСчетов.Найти(МетаданныеРеквизита.Имя) <> Неопределено
					Или Метаданные.ПланыВидовРасчета.Найти(МетаданныеРеквизита.Имя) <> Неопределено Тогда
