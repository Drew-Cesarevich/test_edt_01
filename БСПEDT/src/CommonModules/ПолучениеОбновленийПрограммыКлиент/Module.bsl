...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	Если Не ПараметрыРаботыКлиентаПриЗапуске.ДоступноИспользованиеРазделенныхДанных
		Или Не ПараметрыРаботыКлиентаПриЗапуске.Свойство("ИнтернетПоддержкаПользователей") Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыИПП.Свойство("ПроверкаНаличияОбновления", ПроверкаНаличияОбновления) Тогда
		...
		Если ПроверкаНаличияОбновления.ОписаниеЗадания <> Неопределено Тогда
			ПодключитьОбработкуРезультатаПроверки(
				Новый Структура(ПроверкаНаличияОбновления.ОписаниеЗадания));
